...
> This document is updated against Agent Intercom protocol v4 (`types.ts`, `broker.ts`, and `broker/framing.ts` in the `ctliz` ecosystem), covering Pi `@ctliz/pi-intercom@0.12.1` and Managed Claude `0.13.0-connect.1`. **Written down so we never have to re-derive it.**
>
...
- `@ctliz/pi-intercom` / [ctliz/agent-intercom-pi](https://github.com/ctliz/agent-intercom-pi) (`0.12.1`, protocol v4)
