...
| :--- | :--- | :--- |
| **Pi** | `pi install git:github.com/ctliz/agent-intercom-pi@v0.11.0-connect.2` | Automatic on start; run `/reload` in every open Pi session after install/update. Operates on Agent Intercom protocol v4 with broker-enforced workspace scoping. (npm package `@ctliz/agent-intercom-pi@connect` also available). |
| **Claude Code** | On macOS, install TmuxDeck's pinned Managed Adapter (`0.12.0-connect.3`) from the Create Workspace modal (npm `@ctliz/agent-intercom-claude@connect` = `0.12.0-connect.3`); global npm is not changed. | Choose **Use Managed** (runs with `--tui --safe`) or persistently switch to **Standard Claude**. A global `cci` is left untouched and may still be used as a custom command. |
...
- **Broker-enforced workspace scoping & discovery:** In Agent Intercom protocol v4, `intercom_list({})` is scoped to the current workspace by default and enforced by the broker. Peer names and short ID prefixes resolve strictly within the same workspace scope. Cross-workspace messaging is supported by specifying the **exact full session ID**.
- **Scope is same-OS-user isolation, not a security principal:** Workspace scoping partitions discovery and name resolution to prevent accidental crosstalk; it is an operational routing boundary, not a cryptographic authentication or authorization mechanism. The trust boundary remains the local OS user.
- **Zero raw scope exposure for frontend & mobile:** Desktop and mobile control surfaces maintain zero raw scope exposure (零原值暴露); the backend manages an independent scoped human client per workspace and aggregates them into the unified conversation view.
- **Legacy workspaces fail closed:** Workspaces created under previous protocol revisions or missing scope metadata fail closed on add/rename operations; recreate the workspace to enable full v4 scoping.
- **Coordinated upgrade for installed adapters only:** When updating protocol versions, upgrade only the adapters you have installed on your machine. Run `/reload` in all open Pi sessions and restart other active adapters. You do not need to install unused adapters.
- **Orchestrator:** Orchestrator is an optional Linux/systemd lifecycle product, outside the Broker compatibility set; omitted on macOS.
